    ...
    <LandingPage>
      ...
      <div className='max-w-[1750px] mx-auto w-full flex flex-col gap-8 p-8'>
       ...
       <div className='flex flex-col items-center w-full md:w-1/2 p-2'>
        <span className='text-slate-400 text-lg font-semibold uppercase'>{carInfo?.category}</span>
        <span className='text-primaryGreen text-2xl font-semibold'>{carInfo?.name}</span>
        <p className='text-black opacity-70 leading-relaxed text-sm mt-5 text-center'>{carInfo?.description}</p>
        ...
        <div className='flex flex-wrap gap-6  w-full items-center justify-center' >
          {carInfo?.spec.map((spec, index)=>(
            <div key={index} className='flex gap-2 items-center min-w-20  lg:min-w-60'>
              <span className='h-16 w-16 rounded-full border p-2 flex items-center justify-center'>
              <Icon icon={spec.icon} width="30" height="30" className='text-primaryGreen' />
              </span>
              <span className='flex flex-col justify-center items-center'>
                <span className='text-slate-400 text-sm font-semibold '>{spec.name}</span>
                <span className='text-slate-900 text-xs font-semibold '>{spec.value}</span>
              </span>
            </div>
          ))}
        </div>
